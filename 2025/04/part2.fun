...
public fun part2() {
    ...
    while (_removeCount > 0) {
        _removeCount = 0;
        _newGrid = replicate([$false], 0, _cols);

        for (_row = 1; _row < _rows - 1; ++_row) {
            _newGrid = [_newGrid, $false];

            for (_col = 1; _col < _cols - 1; ++_col) {
                _filled = _grid[_col, _row];
                ...
                _newGrid = [_newGrid, _filled];
            }

            _newGrid = [_newGrid, $false];
        }

        _newGrid = [_newGrid, replicate([$false], 0, _cols)];
        _grid = reshape2(_newGrid, [_cols, _rows]);
        _count += _removeCount;

        /* write(*, "Removed ", _removeCount, " rolls"); */
    }
